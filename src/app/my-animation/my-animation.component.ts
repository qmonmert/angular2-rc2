...
@Component({
  ...
  template: `
    <h1>Animations</h1>
    <div id='config'>
      <button [disabled]='!canBgColorRed' (click)='setRed()'>Red</button>
      <button [disabled]='canBgColorRed'  (click)='setBlue()'>Blue</button>
      <button [disabled]='!canZoomIn'     (click)='zoomIn()'>Zoom In</button>
      <button [disabled]='canZoomIn'      (click)='zoomOut()'>Zoom Out</button>
      <button [disabled]='!canFlyIn'      (click)='flyIn()'>Fly In</button>
      <button [disabled]='canFlyIn'       (click)='flyOut()'>Fly Out</button>
    </div>
